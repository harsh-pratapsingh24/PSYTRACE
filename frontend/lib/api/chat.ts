...
/**
 * Browser: call same-origin Next.js `/api/chat/*` proxies (so JWT is forwarded server-side).
 * Server: call backend directly if this module is ever imported in RSC.
 */
function getChatApiRoot(): string {
  if (typeof window !== "undefined") {
    return "/api/chat";
  }
  return `${getServerBackendUrl()}/chat`;
}
...
export const createChatSession = async (): Promise<string> => {
  try {
    ...
    const response = await fetch(`${getChatApiRoot()}/sessions`, {
      method: "POST",
...
export const sendChatMessage = async (
  ...
  try {
    ...
    const response = await fetch(
      `${getChatApiRoot()}/sessions/${sessionId}/messages`,
      {
        method: "POST",
        headers: getAuthHeaders(),
...
export const getChatHistory = async (
  ...
  try {
    ...
    const response = await fetch(
      `${getChatApiRoot()}/sessions/${sessionId}/history`,
      {
        headers: getAuthHeaders(),
      }
    );
