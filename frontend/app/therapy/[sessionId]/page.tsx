...
export default function TherapyPage() {
  ...
  const [isLoading, setIsLoading] = useState(false); // ← start false, only true during history fetch
  const [isChatLoading, setIsChatLoading] = useState(false); // separate loading for chat area only
  ...
  // Load history for a given sessionId — does NOT affect full-page loading spinner
  ...
  // When sessionId changes (from handleSessionSelect), reload history
  const isFirstMount = useRef(true);
  useEffect(() => {
    if (isFirstMount.current) {
      isFirstMount.current = false;
      return; // skip on first mount — initChat handles it
    }
  ...
  // Load all chat sessions once on mount
  useEffect(() => {
    ...
    try {
      ...
      const aiResponse =
        typeof response === "string"
          ? (() => {
              try { return JSON.parse(response) as Record<string, unknown>; }
              catch { return { response, message: response }; }
            })()
      ...
      setMessages((prev) => [
        ...
        {
          role: "assistant",
          content: detail.length > 280 ? `${detail.slice(0, 280)}…` : `Something went wrong: ${detail}`,
          timestamp: new Date(),
        },
      ]);
      setIsTyping(false);
  ...
  const handleSessionSelect = (selectedSessionId: string) => {
    ...
    setSessionId(selectedSessionId); // triggers useEffect → loadHistory
